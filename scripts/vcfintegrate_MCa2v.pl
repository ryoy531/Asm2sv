...
GetOptions('--list=s' => \$datalist, '--base=s' => \$basevcf, '--thread=i' => \$cpu, '--output_dir=s' => \$rdir, '--help' => \$help);
...
my ($VF, $VF_str, $err_list, $reffasta) = Read_datalist($datalist, "check");
if($err_list > 0){
...
for(my $i = 0; $i < $d; $i++){
	...
	for(my $j = $p0; $j < $p1; $j++){
		if($SID->[$j]){
			...
			my $cmd = "$bin_sub -r $reffasta -b $basevcf -l $list -s $SID->[$j] -o $each_prefix -w $rdir";
...
sub Help{
	my $help_command =<<"EOS";
Usage: 
  /path/to/Vcfintegrate_cactusMG_asm2sv.pl -b [base.vcf] -l [datalist csv] -t [CPU thread]
