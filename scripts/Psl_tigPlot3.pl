...
if($q =~ /y/i || $q =~ /pipe/i){
	...
	if($wgpsldir ne 'null' && -e $qpsl){		# will be deleted (not use)
		#-----------------------------------------------------//
		print "! found [$qpsl]\n";
		$method_eachalign = "false";
		my ($judge_qpsl, $tid, $hpos0, $hpos1, $hposdata) = Search_hintpsl($qpsl, $seqid, $tpos0, $tpos1, $ab0, $ab1, $lth);
		
		if($judge_qpsl eq 'true'){
			print "\n! summarizing last alignment...\n";
			my $qtpref = $tid."_".$hpos0."_".$hpos1;
			my $num_redo = 0;
			...
			my $Rrh = R_plot($dir, $apref, $hposdata, $dbpref, $qpref, $seqid, $tid, $qtpref, $lth, $tpos0, $tpos1, $ab0, $ab1, 0, $outplot, $keep, $note, $hqtarget->{$tid}{len}, $nomask, $hpos0, $hpos1, $hqtarget->{$tid}{seq}, $tstrand, $neighbor_tlen, $flen, $window_set_factor, $num_redo, $th_border);
			
			if($Rrh->{judge} && $Rrh->{judge} eq 'retry' && $num_redo < 3){
				...
				if(! -e $afile){
					my $abhit_summary = $abhit_header.$Rrh->{abhit_summary};
					SAVE($afile, $abhit_summary);
				}
				else{
					APPEND($afile, $Rrh->{abhit_summary});
				}
			}
		}
		else{
			if($skip_alignmethod eq 'true'){
				print "! no candidate found...\n";
				$method_eachalign = "false";
			}
			else{
				print "! no candidate found, try another method...\n";
				$method_eachalign = "true";
			}
		}
	}
		...
		if($nohit ne 'true'){
			...
			foreach my $AL (@{$AoAL}){
				...
				if($hqtarget->{$tid}{seq}){
					...
					my $Rrh = R_plot($dir, $apref, $hposdata->{$tid}, $dbpref, $qpref, $seqid, $tid, $qtpref, $lth, $tpos0, $tpos1, $ab0, $ab1, 0, $outplot, $keep, $note, $AL->[6], $nomask, $AL->[4], $AL->[5], $hqtarget->{$tid}{seq}, $tstrand, $neighbor_tlen, $flen, $window_set_factor, $num_redo, $th_border);
					...
					my $Rrh = R_plot($dir, $apref, $hposdata->{$tid_alt}, $dbpref, $qpref, $seqid, $tid_alt, $qtpref, $lth, $tpos0, $tpos1, $ab0, $ab1, 0, $outplot, $keep, $note, $AL->[6], $nomask, $AL->[4], $AL->[5], $hqtarget->{$tid_alt}{seq}, $tstrand, $neighbor_tlen, $flen, $window_set_factor, $num_redo, $th_border);
