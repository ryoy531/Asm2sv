...
if(-e $afile){
	...
	if(! -e $rfile_nBDH1){
		...
		if(-e $qgene_region){
			print "! blast-n for BDH search with [$cpu_blast] threads...\n";
			my $cmd_nBDH = "$bin_NBDH blast_nBDH $dbgff $dbgene_region $qgene_region $qpref 5 $pcnt_th $pcov_th $eval_th $cpu_blast y > ./log/log_nBDH.txt 2>&1";
#			print "! cmd=[$cmd_nBDH]\n";
