...
if($q =~ /y/i){
	...
	my $cnt_bdh2 = 0;
	my $cnt_cfh = 0;
	my $r1 = "ID 1=[".$prefix1."],gene_id [1],Asm2sv_partner [1],ID 2=[".$prefix2."],gene_id [2],Asm2sv_BDH_parter,[1] length,[2] length,[1] alignment length,[2] alignment length,[1] \%identity,[2] \%identity,[1] \%coverage,[2] \%coverage,[1] score,[2] score,[1] Eval,[2] Eval,k,l\n";
	...
	foreach my $id1 (@ID1){
		...
		my $gid1 = $id1;
		my $asm2sv_gid1 = $qpref2."__".$gid1;
		
		unless($info1->{$id1}{num_hit}){
			$r1 .= $id1.",".$gid1.",-,NA,".join(",", @emptyR)."\n";
			next;
		}
	...
	print "! [$cnt_bdh2] bidirectional hits\n";
